                        ...
                        <select class="form-control wallet shadow-none" name="wallet_id">
                            ...
                            @foreach ($wallets as $wallet)
                                <option value="{{ $wallet->id }}" data-currency="{{ $wallet->currency->id }}"
                                    data-code="{{ $wallet->currency->code }}">{{ $wallet->currency->code }} --
                                    ({{ numFormat($wallet->balance) }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-12 my-3 info d-none">
                        <ul class="list-group mt-2">
                            <li class="list-group-item d-flex justify-content-between font-weight-bold">
                                @lang('Withdraw Amount : ')<span class="exAmount"></span></li>

                            <li class="list-group-item d-flex justify-content-between font-weight-bold">
                                @lang('Total Charge : ')<span class="exCharge"></span></li>

                            <li class="list-group-item d-flex justify-content-between font-weight-bold">
                                @lang('Total Amount : ')<span class="total_amount"></span></li>
                        </ul>
...
<script>
    ...
    // Xử lý sự kiện confirm trong modal
    $('#modal-success .confirm').on('click', function () {
        $('#form').submit();
    });
</script>
@endpush
