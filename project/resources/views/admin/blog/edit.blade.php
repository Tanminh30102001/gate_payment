         ...
         <div class="card-body">
            ...
            <form action="{{route('admin.blog.update',$blog->id)}}" enctype="multipart/form-data" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="categorys">{{ __('Languages') }}</label>
                            <select class="form-control  mb-3" id="lang" name="lang" required>
                                <option value="" selected disabled>{{__('Select Language')}}</option>
                                @foreach ($lang as $item)
                                <option value="{{$item->code}}"  {{$item->code == $item->code ? 'selected':''}}>{{$item->language}}</option>
                                @endforeach
                            </select>
                          </div>        
                    </div>
